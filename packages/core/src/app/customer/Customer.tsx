...
import { fetchGLSParcelShops } from '../custom/api/fetchGLSParcelShops';
...
function getCartWeightAndPackages(checkoutService: any) {
    const cart = checkoutService.getState().data.getCart();
    const items = cart?.lineItems.physicalItems || [];

    // Peso in kg dal campo weight del prodotto BigCommerce
    ...
    const nOfPackages = items.reduce(
        (sum: number, item: any) => sum + (item.quantity || 1),
        0
    ) || 1;

    return { weight, nOfPackages };
}
...
const Customer: React.FC<CustomerProps> = ({
    ...
    const handleContinueAsGuest = useCallback(async (formValues: GuestFormValues) => {
        ...
        try {
            ...
            if (formValues.shippingAddress) {
                ...
                // ── GLS ──────────────────────────────────────────────────────────
                const { weight, nOfPackages } = getCartWeightAndPackages(checkoutService);
                console.log(getCartWeightAndPackages)
                await fetchGLSParcelShops(
                    {
                        address1: formValues.shippingAddress.address1,
                        address2: formValues.shippingAddress.address2,
    ...
    const executePaymentMethodCheckoutOrContinue = useCallback(async () => {
        try {
            // ── GLS ──────────────────────────────────────────────────────────
            const currentShipping = checkoutService.getState().data.getShippingAddress();
            if (currentShipping) {
                const { weight, nOfPackages } = getCartWeightAndPackages(checkoutService);
                await fetchGLSParcelShops(
                    {
    ...
    }, [customerData.actions, customerData.data.providerWithCustomCheckout, onContinueAsGuest, analyticsTracker, checkoutService]);
